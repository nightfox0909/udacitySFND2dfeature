...
void matchDescriptors(std::vector<cv::KeyPoint> &kPtsSource, std::vector<cv::KeyPoint> &kPtsRef, cv::Mat &descSource, cv::Mat &descRef,
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType,int normType)
{
    // configure matcher
    ...
    cout <<",M" << matches.size() <<"," << 1000 * t / 1.0 << endl;
}
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType)
{
    ...
    else if (descriptorType.compare("BRIEF") == 0) // BRIEF
    {
      int bt=32;
      extractor = cv::xfeatures2d::BriefDescriptorExtractor::create(bt);
    }
    else if ( descriptorType.compare("ORB") == 0)//ORB
    {
       int nfeatures=500;
      ...
      int firstLevel=0;
      int WTA_K=2;
      cv::ORB::ScoreType scoreType=ORB::HARRIS_SCORE; // HARRIS_SCORE =0 FAST_SCORE =1
      int patchSize=31;
       extractor = cv::ORB::create(nfeatures,scaleFactor,nlevels,edgeThreshold,firstLevel,WTA_K,scoreType,patchSize);
    }
    else if ( descriptorType.compare("FREAK") == 0) //FREAK
    {
      extractor =  cv::xfeatures2d::FREAK::create();
    }
    else if ( descriptorType.compare("AKAZE") == 0) //AKAZE
    {
      cv::AKAZE::DescriptorType descriptor_type=AKAZE::DESCRIPTOR_MLDB;
...
void detKeypointsHarris (vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    ...
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    //cout << "Harris corner detection with n=" << keypoints.size() << " keypoints in " << 1000 * t / 1.0 << " ms | ";
...
void detKeypointsModern (vector<KeyPoint> &keypoints, Mat &img, string detectorType, bool bVis)
{
